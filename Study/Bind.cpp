...
struct Bind_t
{
    ...
    typedef typename std::tuple<typename std::decay<Args>::type...> ArgType;

    typedef typename MakeIndexes<sizeof...(Args)>::type slices;
...
public:
    ...
    {
        return do_call(slices(), std::forward_as_tuple(std::forward<CArgs>(args)...));
    }
